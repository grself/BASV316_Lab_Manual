...
```{r 090.02, fig.width=4.5,fig.height=4,fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}
...
```{r 090.03, fig.width=4.5,fig.height=4,fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}
...
```{r 090.04, fig.width=4.5,fig.height=4,fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}
...
Finally, consider the histogram in the following figure, which is taken from the _peri_ vector in the [rock] data frame. This shows a bi-modal distribution where there are two clear peaks in the data. It is critical for researchers to know that the data are bi-modal before they begin analyzing it since having two modes can cause certain statistical tests to fail.

```{r 090.05, fig.width=4.5,fig.height=4,fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}
...
* Line 2: This is the beginning of the histogram function (it ends on Line 8). For this histogram the _Speed_ variable from the [morley] data frame is specified as the data source for the histogram.
...
* Line 7: This specifies that 10 colors will be used from the "cm.color" palette to shade the various bars in the histogram. Researchers need to experiment a bit with the color palette and number of colors to get the best result; however, "cm.color" along with the number of bars in the histogram seems to work well for this histogram. (Note: setting colors on graphs was introduced in [Color].)
...
### Guided Practice: Histogram

Using the [USJudgeRatings] data frame, create a histogram of the _CONT_ vector. The histogram should have these characteristics:

* Main title: _US Judge Ratings_
* X-axis label: _Integrity Rating_
* Y-axis label: _Frequency_
* Breaks: 11
* Colors: cm.colors(8)

```{r 090.07, ex="sc090.01", type="pre-exercise-code"}

# No pre-exercise code for this exercise

```

```{r 090.08, ex="sc090.01", type="sample-code"}
# Create the histogram as specified above.


```

```{r 090.09, ex="sc090.01", type="solution"}
...
Using the [cafe] data frame, create a histogram of _age_. The histogram should:

* Main title: _Activity 8.1: Histogram_
* X-axis label: _Age_
* Y-axis label: _Frequency_
* Breaks: 10
* Colors: topo.colors(12)

Copy/paste the histogram in the deliverable document for this lab.

```{r 090.11, ex="act090.01", type="pre-exercise-code"}

cafe <- read.csv('https://labs.basv316.com/cafe.csv')

```

```{r 090.12, ex="act090.01", type="sample-code"}
...
```{r 090.13, fig.width=4.5,fig.height=4,fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}
...
```{r 090.14, fig.width=4.5,fig.height=4,fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}
...
The following script creates a density plot of the _speed_ variable in the [morley] data frame. The script is very similar to the last script except _R_ plots the density of the data in Line 2 rather than just a count of the various bins.
...
The following script creates a density plot for the _Rape_ vector in the [USArrests] data frame. 
...
### Guided Practice: Density Plot

Using the [USJudgeRatings] data frame, create a density plot of the _CONT_ vector. The plot should have these characteristics:
...
```{r 090.17, ex="sc090.02", type="pre-exercise-code"}

# No pre-exercise code for this exercise

```

```{r 090.18, ex="sc090.02", type="sample-code"}
# Create the density plot as specified above.


```

```{r 090.19, ex="sc090.02", type="solution"}
...
Using the [cafe] data frame, create a density plot of _age_. The plot should:

* Main title: _Activity 8.2: Density Plot_
* X-axis label: _Age_
* Y-axis label: _Density_
* Lwd: 2
* Color: _cadetblue4_

Copy/paste the density plot in the deliverable document for this lab.

```{r 090.21, ex="act090.02", type="pre-exercise-code"}

cafe <- read.csv('https://labs.basv316.com/cafe.csv')

```

```{r 090.22, ex="act090.02", type="sample-code"}
...
```{r 090.23, fig.width=4.5,fig.height=4,fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}
...
```{r 090.24, fig.width=6,fig.height=4,fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}
...
```{r 090.25, fig.width=6,fig.height=4,fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}
...
* Line 6: This specifies the type of line graph wanted. The possible values are: 
    * _p_ for points
    * _l_ for lines (this is a lower-case "L")
    * _b_ for both
    * _c_ for the lines part alone of "b"
    * _o_ for both ’overplotted’ (that is a lower-case "O", not zero)
    * _h_ for 'histogram-like' (or 'high-density') vertical lines
    * _s_ for stair steps (that is a lower-case "S")
    * _S_ for other steps (that is a capital "S") 
    * _n_ for no plotting  
...
### Guided Practice: Line Graph

Using the [UKgas] data frame, create a line graph that meets this specification:

* Plot: [UKgas]
* Main title: _UK Quarterly Gas Consumption_
* X-axis label: _Year_
* Y-axis label: _Millions of Therms_
* Type: l (this is a lower-case "L")
* Lwd: 2
* Color: _lightskyblue_


```{r 090.27, ex="sc090.03", type="pre-exercise-code"}

# No pre-exercise code for this exercise

```

```{r 090.28, ex="sc090.03", type="sample-code"}
# Create a line graph using the specifications listed above.


```

```{r 090.29, ex="sc090.03", type="solution"}
...
* Plot: [lynx]
* Main title: _Activity 8.3: Line Graph_
* X-axis label: _Year_
* Y-axis label: _Number_
* Type: l (this is a lower-case "L")
* Lwd: 2
* Color: _firebrick4_
...
```{r 090.31, ex="act090.03", type="pre-exercise-code"}

cafe <- read.csv('https://labs.basv316.com/cafe.csv')

```

```{r 090.32, ex="act090.03", type="sample-code"}
...
```{r 090.33, fig.width=4.5,fig.height=4,fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}
...
```{r 090.34, fig.width=4.5,fig.height=4,fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}
...
```{r 090.35, fig.width=4.5,fig.height=4,fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}
...
_R_ has 25 different types of symbols that could be used for the points on a plot as shown on the following chart.

```{r 090.36, fig.width=4.5,fig.height=4,fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}

# this script found at http://www.sthda.com/english/wiki/r-plot-pch-symbols-the-different-point-shapes-available-in-r
#++++++++++++++++++++++++++++++++++++++++++++
...
* Line 2: This starts the `plot` function. Like many _R_ functions, `plot` requires a formula input in the form of _y ~ x_ where the dependent variable (the y-axis) is first in the formula and the independent variable (the x-axis) is second. For this plot, _Education_ is the independent variable and will be on the x-axis while _Fertility_ is the dependent variable on the y-axis. The researcher was answering the question "Does education level affect the number of children people have?"
* Lines 3-5: This creates the main title and the labels for the x-axis and y-axis.
* Line 6: The _pch_ attribute is the type of point used on the graph. _R_ has a number of pre-defined types of points, as illustrated above, and for this graph type number _21_ is selected.
* Line 7: The background color of the points is specified using the _bg_ attribute. This this case it is a shade of chartreuse, a yellow-green hue. 
* Line 8: The line color of the circle is specified using the _col_ attribute.
...
* Lines 2-9: This demonstration plots two vectors from the [attitude] data frame, _rating_ is the independent variable on the x-axis while _complaints_ is the dependent variable on the y-axis. The researcher was answering the question "Do employees with higher overall ratings handle complaints better?" The other  parameters of this `plot` function are defined for the previous plot and are not further discussed here.
...
### Guided Practice: Plot

Using the [rock] data frame, create a plot of _area_ (y-axis) as a function of _peri_ (x-axis). The graph should use these specifications:

Plot:

* Main title: _Rock Area by Perimeter_
* X-axis label: _Perimeter_
* Y-axis label: _Area_
* Pch: 21
* Bg: _goldenrod3_
* Color: _black_
...
```{r 090.39, ex="sc090.04", type="pre-exercise-code"}

# No pre-exercise code for this exercise

```

```{r 090.40, ex="sc090.04", type="sample-code"}
# Create a scatter plot with a line of best fit as specified above.


```

```{r 090.41, ex="sc090.04", type="solution"}
...
Using the [cafe] data frame, create a simple plot with _length_ on the x-axis, _bill_ on the y-axis, and a line of best fit to see if there is a relationship between the length of the meal and the bill. The plot should:

Plot:

* Main title: _Activity 8.4: Line of Best Fit_
* X-axis label: _Length of Meal_
* Y-axis label: _Bill_
* Pch: 23
* Bg: _indianred4_
* Color: _black_
...
```{r 090.43, ex="act090.04", type="pre-exercise-code"}

cafe <- read.csv('https://labs.basv316.com/cafe.csv')

```

```{r 090.44, ex="act090.04", type="sample-code"}
...
```{r 090.45, fig.width=4.5,fig.height=4,fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}
...
```{r 090.46, fig.width=4.5,fig.height=4,fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}
...
```{r 090.47, fig.width=4.5,fig.height=4,fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}
...
* Lines 2-4: This executes the `qqnorm` function and passes that function the weight variable from the [chickwts] data frame. This function draws the Q-Q Plot. The only parameter needed is _main_, which adds the title to the plot.
...
### Guided Practice: Q-Q Plot

Using the [swiss] data frame, create a Q-Q plot of the _Fertility_ vector. The plot should have a main title of _QQ Plot: Swiss Fertility_ and a _red_ Q-Q line with a width of 2.

```{r 090.49, ex="sc090.05", type="pre-exercise-code"}

# No pre-exercise code for this exercise

```

```{r 090.50, ex="sc090.05", type="sample-code"}
# Create a Q-Q plot of the Fertility vector in the swiss data frame using the requirements listed above.


```

```{r 090.51, ex="sc090.05", type="solution"}
...
Using the [cafe] data frame, create a Q-Q plot of _age_. The plot should h ave a title of _Activity 8.5: Q-Q Plot_ and a _sienna4_ Q-Q line with a width of 2.

Copy/paste the plot in the deliverable document for this lab.

```{r 090.53, ex="act090.05", type="pre-exercise-code"}

cafe <- read.csv('https://labs.basv316.com/cafe.csv')

```

```{r 090.54, ex="act090.05", type="sample-code"}
