...
R makes it easy to calculate various data descriptives, as covered in the [dispersion tutorial](/tutorials/0030-dispersion); however, most people find it easier to understand data descriptives when those data are presented graphically. Fortunately, _R_ has a great graphic tool for visualizing data descriptives: Boxplot (sometimes called a “Box and Whisker” plot). A Boxplot graphically illustrates Q1, the mean, the median, Q3, outlier boundaries, and outliers (if any are present).

-----

## About Visualizations

This site includes several tutorials that are focued on data visualization because it is a critically important tool for analysis. Visualizations are useful in two different phases of the analysis process: exploration and explanation. In the exploration phase, researchers are looking for interesting relationships in the data and those relationships are often difficult to detect in a table full of numbers but a visualization makes them instantly clear. As an example, here are two ways to look at the _Volume_ vector in the [_trees_](/tutorials/9999-data-dict/index.html#trees) data frame.
...
```{r fig04.01, fig.dim=c(2.5,4.0), fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}
...
Following is the summary data for _hp_ from the [_mtcars_](/tutorials/9999-data-dict/index.html#mtcars) data frame along with the boxplot for that same data.

```{r fig04.02, fig.dim=c(2.5,4.0), fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}
...
The following script generates four different boxplots for four different variables in the [_rock_](/tutorials/9999-data-dict/index.html#rock) data frame.
...
* Lines 5-15 These are repetitions of Lines 1-3 for three other vectors in the _rock_ data frame.

<span style="background-color: #ffff99; font-style: italic;">This demonstration contains four different examples of generating a summary and a boxplot.</span>
...
Using the [_iris_](/tutorials/9999-data-dict/index.html#iris) data frame, create a boxplot for _Sepal.Length_.

```{r ex="ex04.01", type="pre-exercise-code"}

# No pre-exercise code for this exercise

```

```{r ex="ex04.01", type="sample-code"}
# Create a boxplot for Sepal.Length


```

```{r ex="ex04.01", type="solution"}

boxplot(iris$Sepal.Length)

```

```{r ex="ex04.01", type="sct"}
...
### Activity 4.1: Boxplot

Using the _cafe_ data frame, generate a boxplot for both _age_. Copy/paste the boxplot in the deliverable document for this lab.

```{r ex="act04.01", type="pre-exercise-code"}

cafe <- read.csv('https://gself.com/cafe.csv')

```

```{r ex="act04.01", type="sample-code"}
...
Using the _iris_ data frame, use boxplot.stats to see if _Sepal.Width_ has any outliers.

```{r ex="ex04.02", type="pre-exercise-code"}

# No pre-exercise code for this exercise

```

```{r ex="ex04.02", type="sample-code"}
# Use boxplot.stats to see if Sepal.Width has any outliers

```

```{r ex="ex04.02", type="solution"}

boxplot.stats(iris$Sepal.Width)

```

```{r ex="ex04.02", type="sct"}
...
### Activity 4.2: Outliers

Using the _cafe_ data frame, find the outliers for _miles_. Record those outliers in the deliverable document for this lab.

```{r ex="act04.02", type="pre-exercise-code"}

cafe <- read.csv('https://gself.com/cafe.csv')

```

```{r ex="act04.02", type="sample-code"}
...
```{r fig04.04, fig.dim=c(4.0, 4.0), fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}
...
_R_ permits designers to use a number of different color palettes to make a graph easier to understand. Here are the palettes available in base _R_, as used in this tutorial. _Note: R can use any of the millions of colors available on a computer screen, but the five fundamental palettes shown below are easy to use and will serve analysts well in many projects._

```{r fig04.05, fig.dim=c(2.5,4.0), fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}
...
Adding the _heat_ color palette to the grouped boxplot generated earlier in this tutorial makes it easier to read.

```{r fig04.06, fig.dim=c(4.0, 4.0), fig.align="center", echo=FALSE, message=FALSE, warnings=FALSE, tut=FALSE}
...
* Lines 7-11: These are similar to Lines 2-5 but use the [_warpbreaks_](/tutorials/9999-data-dict/index.html#warpbreaks) data frame. Notice that selecting the "heat" color palette is slightly different than the rainbow palette used above.
* Lines 13-18: These lines are similar to Lines 7-11 but use the [_chickwts_](/tutorials/9999-data-dict/index.html#chickwts) data frame. A new attribute was specified: _las = 2_. For this boxplot the groups are names of chicken feed rather than numbers or single letters. When those names are printed horizontally they “run into each other” and become unreadable. The _las = 2_ specification turns those labels 90&deg; so they do not interfere with each other.

<span style="background-color: #ffff99; font-style: italic;">This demonstration contains three different examples of generating grouped boxplots with color.</span>
...
Using the [_morley_](/tutorials/9999-data-dict/index.html#morley) data frame, generate a grouped boxplot where _Speed_ is grouped by _Expt_. Set the _main_ title to "Morley Experiment" and the color to _rainbow(5)_.

```{r ex="ex04.03", type="pre-exercise-code"}

# No pre-exercise code for this exercise

```

```{r ex="ex04.03", type="sample-code"}
...
### Activity 4.3: Grouped Boxplot With Color

Using the _cafe_ data frame, generate a boxplot of _length_ when grouped by _meal_ to see if there is any difference in the length of the meal by the type of meal eaten (breakfast, lunch, dinner, other). Apply the _topo_ color palette to this boxplot. Copy/paste the boxplot in the deliverable document for this lab.

```{r ex="act04.03", type="pre-exercise-code"}

cafe <- read.csv('https://gself.com/cafe.csv')

```

```{r ex="act04.03", type="sample-code"}
...
This tutorial created a visualization for various [central measures](/tutorials/0020-central-measures) and [measures of dispersion](/tutorials/0030-dispersion). The next tutorial is still being built and is not yet available.
