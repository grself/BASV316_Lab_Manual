...
Using the _cafe_ data, conduct an ANOVA and report the p-value for the following variables. Note: in the document submitted for this lab, Activity 9.1 should have a simple listing, something like illustrated here. (Notes: these are not the correct answers to the listed tests. To indicate tiny values use scientific notation in a form like 1.6e-05 since that is easier to type.) Record the p-values in the deliverable document for this lab.
...
```{r ex="act09.01", type="pre-exercise-code"}

cafe <- read.csv('https://labs.basv316.com/cafe.csv')

```

```{r ex="act09.01", type="sample-code"}
...
```{r ex="ex09.02", type="pre-exercise-code"}

# No pre-exercise code for this exercise

```

```{r ex="ex09.02", type="sample-code"}
# Calculate a t-test for yield when grouped by N in the npk data frame.


```

```{r ex="ex09.02", type="solution"}

t.test(yield ~ N, data = npk)

```

```{r ex="ex09.02", type="sct"}
...
```{r ex="act09.02", type="pre-exercise-code"}

cafe <- read.csv('https://labs.basv316.com/cafe.csv')

```

```{r ex="act09.02", type="sample-code"}
