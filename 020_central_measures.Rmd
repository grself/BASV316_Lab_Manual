...
<div class="grsnote">A _vector_ is nothing more than a list of related items. For example, a vector of street names may look like {"elm", "main", "first", "raven"} and a vector of test scores may look like {83, 91, 76, 88}.</div>
...
* Line 1: Any line in _R_ that starts with a hash is a comment and is ignored when the script is executed. _(Fun fact, the official name for that symbol is "octothorpe".)_
* Line 2: The length of the _eruptions_ vector in the [faithful] data frame is printed, which is the same as the number of items in the vector, or _N_. **Notice how a vector is identified using the data frame name then the vector name with a dollar sign operator.**
...
```{r 020.03, ex="sc020.01", type="pre-exercise-code"}

# No pre-exercise code for this exercise

```

```{r 020.04, ex="sc020.01", type="sample-code"}
# Find N for 'critical' in the 'attitude' data frame

```


```{r 020.05, ex="sc020.01", type="solution"}

length(attitude$critical)

```

```{r 020.06, ex="sc020.01", type="sct"}
...
```{r 020.07, ex="act020.01", type="pre-exercise-code"}

cafe <- read.csv('https://labs.basv316.com/cafe.csv')

```

```{r 020.08, ex="act020.01", type="sample-code"}
...
* Line 1: Any line in _R_ that starts with a hash is a comment and is ignored when the script is executed.
...
### Guided Practice: Mean

Using the [attitude] data frame, find the mean for _critical_.

```{r 020.10, ex="sc020.02", type="pre-exercise-code"}

# No pre-exercise code for this exercise

```

```{r 020.11, ex="sc020.02", type="sample-code"}
...
```{r 020.12, ex="sc020.02", type="solution"}

mean(attitude$critical)

```

```{r 020.13, ex="sc020.02", type="sct"}
...
```{r 020.14, ex="act020.02", type="pre-exercise-code"}

cafe <- read.csv('https://labs.basv316.com/cafe.csv')

```

```{r 020.15, ex="act020.02", type="sample-code"}
...
* Line 1: Any line in _R_ that starts with a hash is a comment and is ignored when the script is executed.
* Lines 2-4: calculate the trimmed mean by including "trim=..." in the `mean` function. Line 2 is the mean for the _accel_ vector in the [attenu] data frame, Line 3 trims 15% from each end of the vector and Line 4 trims 25%.
...
### Guided Practice: Trimmed Mean

Using the [attitude] data frame, find the mean for _critical_ with 15% trimmed.

```{r 020.18, ex="sc020.03", type="pre-exercise-code"}

# No pre-exercise code for this exercise

```

```{r 020.19, ex="sc020.03", type="sample-code"}
...
```{r 020.20, ex="sc020.03", type="solution"}

mean(attitude$critical, trim=0.15)

```

```{r 020.21, ex="sc020.03", type="sct"}
...
```{r 020.22, ex="act020.03", type="pre-exercise-code"}

cafe <- read.csv('https://labs.basv316.com/cafe.csv')

```

```{r 020.23, ex="act020.03", type="sample-code"}
...
### Guided Practice: Median

Using the [attitude] data frame, find the median for _critical_.

```{r 020.25, ex="sc020.04", type="pre-exercise-code"}

# No pre-exercise code for this exercise

```

```{r 020.26, ex="sc020.04", type="sample-code"}
...
```{r 020.27, ex="sc020.04", type="solution"}

median(attitude$critical)

```

```{r 020.28, ex="sc020.04", type="sct"}
...
```{r 020.29, ex="act020.04", type="pre-exercise-code"}

cafe <- read.csv('https://labs.basv316.com/cafe.csv')

```

```{r 020.30, ex="act020.04", type="sample-code"}
...
```{r echo=FALSE, message=FALSE, warnings=FALSE, results='asis', tut=FALSE}
...
Complete the deliverables in this lab and consolidate the responses into a single document. Name the document with your name and "Lab 2," like "George Self Lab 2" and submit that document for grade.
